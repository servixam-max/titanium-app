...
import { Flame, Dumbbell, Clock, TrendingUp, Calendar, Award, Activity, ArrowLeft } from "lucide-react";
...
function calculateStreak(sessions: WorkoutSession[]) {
  ...
  const dates = Array.from(new Set(completed.map((s) => new Date(s.endTime!).toDateString()))).map((d) => new Date(d));
...
export default function StatsPage() {
  ...
  const totalSets = completed.reduce((sum, s) => sum + s.exercises.reduce((e, ex) => e + ex.sets.length, 0), 0);
  const totalReps = completed.reduce(
    (sum, s) => sum + s.exercises.reduce((e, ex) => e + ex.sets.reduce((se, set) => se + (set.reps || 0), 0), 0),
    0
  );
  const totalVolume = completed.reduce(
    (sum, s) =>
      sum +
      s.exercises.reduce(
        (e, ex) => e + ex.sets.reduce((se, set) => se + (set.weight || 0) * (set.reps || 0), 0),
        0
      ),
    0
  );
  ...
  const thisWeek = completed.filter((s) => s.endTime && new Date(s.endTime) >= weekStart).length;

  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  const thisMonth = completed.filter((s) => s.endTime && new Date(s.endTime) >= monthStart).length;
  ...
  const chartData = completed.slice(-8).map((s) => ({
    start_time: typeof s.startTime === "string" ? s.startTime : s.startTime.toISOString(),
    ...
    <div className="min-h-screen pb-[120px]">
      ...
      <main className="w-full px-container-padding pt-4 flex flex-col gap-section-gap">
        ...
        <section className="grid grid-cols-2 gap-3">
          <StatCard icon={<Dumbbell className="w-5 h-5 text-primary-container" />} label="SESIONES" value={total.toString()} sublabel={`${thisWeek} esta semana`} />
          <StatCard icon={<TrendingUp className="w-5 h-5 text-primary-container" />} label="VOLUMEN" value={`${(totalVolume / 1000).toFixed(1)}T`} sublabel={`${totalSets} series totales`} />
          <StatCard icon={<Clock className="w-5 h-5 text-primary-container" />} label="TIEMPO TOTAL" value={formatDuration(totalDuration)} sublabel={`Media ${formatDuration(avgDuration)}`} />
          <StatCard icon={<Award className="w-5 h-5 text-primary-container" />} label="REPETICIONES" value={totalReps.toString()} sublabel={`Media ${(avgVolume / 1000).toFixed(1)}T/ses`} />
        </section>
        ...
        <section className="bg-surface-container-low border border-surface-container-highest rounded-xl p-4">
          ...
          <div className="flex justify-between items-center py-2 border-b border-surface-container-highest">
            <span className="font-body-md text-body-md text-on-surface-variant">Esta semana</span>
            <span className="font-bold text-primary-container">{thisWeek} sesiones</span>
          </div>
          <div className="flex justify-between items-center py-2 border-b border-surface-container-highest">
            <span className="font-body-md text-body-md text-on-surface-variant">Este mes</span>
            <span className="font-bold text-primary-container">{thisMonth} sesiones</span>
          </div>
          <div className="flex justify-between items-center py-2">
            <span className="font-body-md text-body-md text-on-surface-variant">Última sesión</span>
            <span className="font-bold text-primary-container capitalize">
              {lastSession
                ? safeFormatDate(typeof lastSession === "string" ? lastSession : lastSession.toISOString(), {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })
...
function StatCard({
  ...
  return (
    <div className="bg-surface-container-low border border-surface-container-highest rounded-xl p-3 flex flex-col gap-2">
      ...
      <div>
        <span className="font-headline-lg text-headline-lg text-primary-container block leading-none">{value}</span>
        <span className="font-label-caps text-[10px] text-on-surface-variant mt-1 block">{sublabel}</span>
      </div>
    </div>
  );
}

function VolumeChart({ data }: { data: Array<{ start_time: string; volume: number; routine_name: string }> }) {
  const width = 400;
  const height = 200;
  ...
  const getY = (vol: number) => padding.top + chartHeight - (vol / maxVol) * chartHeight;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ maxHeight: 200 }}>
      {[0, 0.5, 1].map((f, i) => {
        ...
        return <line key={i} x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="#262626" strokeWidth="1" />;
      })}
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={y} width={barWidth} height={h} fill="#ccff00" opacity={i === data.length - 1 ? 1 : 0.7} rx="3" />
            {i === data.length - 1 && vol > 0 && (
              <text x={x + barWidth / 2} y={y - 6} textAnchor="middle" fill="#ccff00" fontSize="9" fontWeight="bold">
                {(vol / 1000).toFixed(1)}T
              </text>
            )}
            <text x={x + barWidth / 2} y={height - 10} textAnchor="middle" fill="#666" fontSize="8">
